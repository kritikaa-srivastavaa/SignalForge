...
export function PageHeader({ title, description, onRefresh, loading }: {
  ...
  return <header className="page-header"><div><p className="eyebrow">WORKSPACE / {title.toUpperCase()}</p>
...
export function SeverityBadge({ value }: { value: string }) {
  ...
  return <span className={`badge ${tone}`}><span className="badge-dot" aria-hidden="true" />{value}</span>;
}
...
export function LoadingState() {
  return <div className="state" role="status"><span className="loading-mark" aria-hidden="true">•••</span><strong>Loading workspace data…</strong><p>Retrieving the latest records.</p></div>;
}
...
export function PaginationControls({ page, noun, onPage }: { page: PageResponse<unknown>; noun: string; onPage: (value: number) => void }) {
  return <div className="pagination"><span><strong>{formatCount(page.totalElements)}</strong> {noun}</span>
