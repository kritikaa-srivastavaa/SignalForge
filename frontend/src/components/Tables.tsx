...
export function EventsTable({ events, compact = false }: { events: Event[]; compact?: boolean }) {
  return <div className="table-scroll" tabIndex={0} role="region" aria-label="Events table"><table>
    ...
    <tbody>{events.map(event => <tr key={event.id}><td className="service" title={`Event ID: ${event.id}`}>{event.service}</td><td className="mono">{event.type}</td><td><SeverityBadge value={event.severity} /></td>
